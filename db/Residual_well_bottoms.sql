...
FROM
    TIG_WELL_HISTORY twh left join TIG_COMPUTED_DEVIATION cd on cd.TIG_WELL_SLDNID = twh.DB_SLDNID
WHERE
    (NOT twh.TIG_LONGITUDE = 0
    OR NOT twh.TIG_LATITUDE = 0)
    AND(cd.DB_SLDNID IS NULL
    OR cd.DB_SLDNID IN
    (SELECT
        MAX(cd2.DB_SLDNID)
    FROM
    ...
    WHERE
        cd2.TIG_WELL_SLDNID = twh.DB_SLDNID
    ))
    AND EXISTS
    (SELECT
        *
