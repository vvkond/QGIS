...
FROM
    ...
    FROM
        ...
        left join (select TIG_USER_ID, max(TIG_LOGIN_NAME) as TIG_LOGIN_NAME from tig_interpreter group by TIG_USER_ID) ii on w.TIG_INTERPRETER_SLDNID = ii.TIG_USER_ID,
		well b
    WHERE
