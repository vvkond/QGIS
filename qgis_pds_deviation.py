...
def ramerdouglas(line, dist):
    ...
    for curr in line[1:-1]:
        if float(_vec2d_dist(begin, end))<>float(0):
            tmp = (
...
class QgisPDSDeviation(QObject, WithQtProgressBar):
    def __init__(self, iface, project  ,styleName=None ,styleUserDir=None,simplificationDistance=0.1):
        '''
            @param simplificationDistance: if not None then use Ramer-Douglas-Peucker simplification of a curve with that distance
        '''
        ...
        self.simplificationDistance=simplificationDistance
        ...
        with edit_layer(layer):
            for idx,row in enumerate(dbWells):
                ...
                if (filterWellIds is not None) and (wellId not in filterWellIds):
                    ...
                    line=numpy.stack([blob_x,blob_y]).T.tolist()
                    if self.simplificationDistance:
                        line=ramerdouglas(line=line,dist=self.simplificationDistance)
                    for (dx,dy) in line:
                        pt = QgsPoint(startX+dx, startY+dy)
                        polyLine.append(pt)
                    pt = QgsPoint(pt.x()+0.00000001, pt.y()+0.00000001) #special offset for draw wel without deviation
                    polyLine.append(pt)
