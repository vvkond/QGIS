...
import matplotlib2.backends.tkagg as tkagg

from matplotlib2.backends.backend_agg import FigureCanvasAgg
import matplotlib2.backends.windowing as windowing

import matplotlib2
from matplotlib2 import backend_tools, cbook, rcParams
...
def error_msg_tkpaint(msg, parent=None):
    ...
    tkMessageBox.showerror("matplotlib2", msg)
...
class _BackendTk(_Backend):
    ...
    def new_figure_manager_given_figure(cls, num, figure):
        ...
        """
        _focus = windowing.FocusManager()
        window = Tk.Tk(className="matplotlib2")
        window.withdraw()
        ...
        icon_fname = os.path.join(
            rcParams['datapath'], 'images', 'matplotlib2.ppm')
        icon_img = Tk.PhotoImage(file=icon_fname)
        try:
            window.tk.call('wm', 'iconphoto', window._w, icon_img)
        except Exception as exc:
            # log the failure (due e.g. to Tk version), but carry on
            _log.info('Could not load matplotlib2 icon: %s', exc)
