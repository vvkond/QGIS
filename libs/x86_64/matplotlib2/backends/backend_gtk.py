...
from matplotlib2.backends.backend_gdk import RendererGDK, FigureCanvasGDK
...
class FigureCanvasGTK (gtk.DrawingArea, FigureCanvasBase):
    ...
    def __init__(self, figure):
        if self.__class__ == matplotlib2.backends.backend_gtk.FigureCanvasGTK:
            warn_deprecated('2.0', message="The GTK backend is "
                            "deprecated. It is untested, known to be "
                            "broken and will be removed in matplotlib2 3.0. "
                            "Use the GTKAgg backend instead. "
                            "See matplotlib2 usage FAQ for"
    ...
    def button_press_event(self, widget, event):
        ...
        if not dblclick:
            ...
            # sequence.  In order to provide consistency to matplotlib2 users, we will
...
class FigureManagerGTK(FigureManagerBase):
    ...
    def __init__(self, canvas, num):
        ...
        self.window.set_wmclass("matplotlib2", "matplotlib2")
        ...
        if window_icon:
            try:
                ...
                # better way is - JDH
                _log.info('Could not load matplotlib2 '
    ...
    def destroy(self, *args):
        ...
        if Gcf.get_num_fig_managers()==0 and \
               not matplotlib2.is_interactive() and \
               gtk.main_level() >= 1:
            gtk.main_quit()
...
class DialogLineprops(object):
    ...
    def __init__(self, lines):
        ...
        datadir = matplotlib2.get_data_path()
...
try:
    if gtk.pygtk_version < (2, 8, 0) or sys.platform == 'win32':
        icon_filename = 'matplotlib2.png'
    else:
        icon_filename = 'matplotlib2.svg'
    window_icon = os.path.join(rcParams['datapath'], 'images', icon_filename)
except:
    window_icon = None
    _log.info('Could not load matplotlib2 icon: %s', sys.exc_info()[1])
