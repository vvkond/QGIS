...
backend = matplotlib2.get_backend()
...
def pylab_setup(name=None):
    ...
    name : str, optional
        ...
        ``matplotlib2.get_backend()`` (which return :rc:`backend`).
    ...
    if name is None:
        ...
        name = matplotlib2.get_backend()
    if name.startswith('module://'):
        backend_name = name[9:]
    else:
        backend_name = 'backend_' + name
        backend_name = backend_name.lower()  # until we banish mixed case
        backend_name = 'matplotlib2.backends.%s' % backend_name.lower()
...
Please select a GUI backend in your matplotlib2rc file ('%s')
or with matplotlib2.use()""" %
                          (name, matplotlib2.matplotlib2_fname()))
    ...
    # reasons. See https://github.com/matplotlib2/matplotlib2/issues/6092
    global backend
    backend = name
