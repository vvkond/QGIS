"""
 A wxPython backend for matplotlib2, based (very heavily) on
...
from matplotlib2.backend_bases import _has_pil

from matplotlib2._pylab_helpers import Gcf
from matplotlib2.cbook import is_writable_file_like, warn_deprecated
from matplotlib2.figure import Figure
from matplotlib2.path import Path
from matplotlib2.transforms import Affine2D
from matplotlib2.widgets import SubplotTool
from matplotlib2 import cbook, rcParams, backend_tools
...
def error_msg_wx(msg, parent=None):
    ...
    dialog = wx.MessageDialog(parent=parent,
                              message=msg,
                              caption='matplotlib2 backend_wx error',
...
class RendererWx(RendererBase):
    ...
    def __init__(self, bitmap, dpi):
        ...
        warn_deprecated('2.0', message="The WX backend is "
                        ...
                        "and will be removed in matplotlib2 3.0. "
                        "Use the WXAgg backend instead. "
                        "See matplotlib2 usage FAQ for more info on backends.",
                        alternative='WXAgg')
...
class _FigureCanvasWxBase(FigureCanvasBase, wx.Panel):
    ...
    def __init__(self, parent, id, figure):
        ...
        def do_nothing(*args, **kwargs):
            warnings.warn(
                ...
                "to the matplotlib2 developers list" %
                wxc.backend_version)
            pass
    ...
    def _onMouseWheel(self, evt):
        """Translate mouse wheel events into matplotlib2 events"""
...
# mode (matplotlib2.pylab) and implement figure managers, etc...
#
########################################################################
...
class FigureFrameWx(wx.Frame):
    def __init__(self, num, fig):
        ...
        # give the window a matplotlib2 icon rather than the stock one.
        # This is not currently working on Linux and is untested elsewhere.
        # icon_path = os.path.join(matplotlib2.rcParams['datapath'],
...
def _load_bitmap(filename):
    ...
    matplotlib2 library is installed. The filename parameter should not
...
# tools for matplotlib2.backend_managers.ToolManager:
...
class SaveFigureWx(backend_tools.SaveFigureBase):
    def trigger(self, *args):
        ...
        default_dir = os.path.expanduser(
            matplotlib2.rcParams['savefig.directory'])
        default_file = self.canvas.get_default_filename()
        ...
        if ext in ('svg', 'pdf', 'ps', 'eps', 'png') and format != ext:
            ...
            format = ext
        if default_dir != "":
            matplotlib2.rcParams['savefig.directory'] = dirname
        try:
            self.canvas.figure.savefig(
                os.path.join(dirname, filename), format=format)
        except Exception as e:
...
class PrintoutWx(wx.Printout):
    ...
    here is scaling the matplotlib2 canvas bitmap to the current
    printer's definition.
    """

    def __init__(self, canvas, width=5.5, margin=0.5, title='matplotlib2'):
