...
def _get_textbox(text, renderer):
    ...
    :meth:`matplotlib2.text.Text.get_extents` method, The bbox size of
...
class Text(Artist):
    ...
    def __init__(self,
        ...
        """
        Create a :class:`~matplotlib2.text.Text` instance at *x*, *y*
        with string *text*.
    ...
    def set_clip_box(self, clipbox):
        """
        Set the artist's clip :class:`~matplotlib2.transforms.Bbox`.

        ACCEPTS: a :class:`matplotlib2.transforms.Bbox` instance
        """
    ...
    def set_clip_path(self, path, transform=None):
        ...
        Set the artist's clip path, which may be:
          ...
          * a :class:`~matplotlib2.patches.Patch` (or subclass) instance

          * a :class:`~matplotlib2.path.Path` instance, in which case
             an optional :class:`~matplotlib2.transforms.Transform`
        ...
        ACCEPTS: [ (:class:`~matplotlib2.path.Path`,
        :class:`~matplotlib2.transforms.Transform`) |
        :class:`~matplotlib2.patches.Patch` | None ]
        """
        ...
        with _wrap_text(self) as textobj:
            ...
            for line, wh, x, y in info:
                ...
                if textobj.get_path_effects():
                    from matplotlib2.patheffects import PathEffectRenderer
    ...
    def get_window_extent(self, renderer=None, dpi=None):
        '''
        Return a :class:`~matplotlib2.transforms.Bbox` object bounding
    ...
    def set_fontproperties(self, fp):
        ...
        :class:`matplotlib2.font_manager.FontProperties` object.

        ACCEPTS: a :class:`matplotlib2.font_manager.FontProperties` instance
        """
...
class TextWithDash(Text):
    """
    This is basically a :class:`~matplotlib2.text.Text` with a dash
    (drawn with a :class:`~matplotlib2.lines.Line2D`) before/after
    it. It is intended to be a drop-in replacement for
    :class:`~matplotlib2.text.Text`, and should behave identically to
    it when *dashlength* = 0.0.

    The dash always comes between the point specified by
    :meth:`~matplotlib2.text.Text.set_position` and the text. When a
    ...
    *dashrotation* specifies the rotation of the dash, and should
    ...
    :meth:`~matplotlib2.text.TextWithDash.get_dashrotation` returns
    :meth:`~matplotlib2.text.Text.get_rotation`.  (i.e., the dash takes
    ...
    *dashpush* "pushes" the dash and text away from the point
    specified by :meth:`~matplotlib2.text.Text.set_position` by the
    amount in canvas units.  (default = 0)

    .. note::

        The alignment of the two objects is based on the bounding box
        of the :class:`~matplotlib2.text.Text`, as obtained by
        :meth:`~matplotlib2.artist.Artist.get_window_extent`.  This, in
    ...
    .. note::
        ...
        :meth:`~matplotlib2.text.TextWithDash.get_window_extent` right,
    ...
    def set_transform(self, t):
        """
        Set the :class:`matplotlib2.transforms.Transform` instance used
        by this artist.

        ACCEPTS: a :class:`matplotlib2.transforms.Transform` instance
        """
    ...
    def set_figure(self, fig):
        ...
        ACCEPTS: a :class:`matplotlib2.figure.Figure` instance
        """
...
class _AnnotationBase(object):
    ...
    def _get_xy_transform(self, renderer, s):
        ...
        if isinstance(s, tuple):
            s1, s2 = s
            from matplotlib2.transforms import blended_transform_factory
        ...
        if s == 'data':
            ...
            from matplotlib2.projections import PolarAxes
        ...
        if xy0 is not None:
            ...
            ref_x, ref_y = xy0
            from matplotlib2.transforms import Affine2D
            if unit == "points":
                # dots per points
    ...
    def draggable(self, state=None, use_blit=False):
        ...
        draggable is on.
        """
        from matplotlib2.offsetbox import DraggableAnnotation
...
class Annotation(Text, _AnnotationBase):
    ...
    def __init__(self, s, xy,
        ...
        Additional kwargs are passed to `~matplotlib2.text.Text`.
        ...
        xycoords : str, Artist, Transform, callable or tuple, optional
            ...
            If a `~matplotlib2.artist.Artist` object is passed in the units are
            fraction if it's bounding box.

            If a `~matplotlib2.transforms.Transform` object is passed
            in use that to transform ``xy`` to screen coordinates

            If a callable it must take a
            `~matplotlib2.backend_bases.RendererBase` object as input
            and return a `~matplotlib2.transforms.Transform` or
            `~matplotlib2.transforms.Bbox` object
        ...
        arrowprops : dict, optional
            ...
            `~matplotlib2.patches.FancyArrowPatch` arrow between ``xy`` and
            ``xytext``.
            ...
            ?            any key to :class:`matplotlib2.patches.FancyArrowPatch`
            ==========   ======================================================
            ...
            Valid keys for `~matplotlib2.patches.FancyArrowPatch` are:
            ...
            ?                any key for :class:`matplotlib2.patches.PathPatch`
            ===============  ==================================================
