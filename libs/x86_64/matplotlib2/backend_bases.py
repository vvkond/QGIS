...
graphics contexts must implement to serve as a matplotlib2 backend
...
:class:`FigureCanvasBase`
    ...
    :class:`matplotlib2.figure.Figure` from the backend specific
...
:class:`Event`
    The base class for all of the matplotlib2 event
    handling.  Derived classes such as :class:`KeyEvent` and
    :class:`MouseEvent` store the meta data like keys and buttons
    pressed, x and y locations in pixel and
    :class:`~matplotlib2.axes.Axes` coordinates.
...
from matplotlib2 import (
    backend_tools as tools, cbook, colors, textpath, tight_bbox, transforms,
    widgets, get_backend, is_interactive, rcParams)
from matplotlib2._pylab_helpers import Gcf
from matplotlib2.transforms import Bbox, TransformedBbox, Affine2D
from matplotlib2.path import Path
...
_default_backends = {
    'ps': 'matplotlib2.backends.backend_ps',
    'eps': 'matplotlib2.backends.backend_ps',
    'pdf': 'matplotlib2.backends.backend_pdf',
    'pgf': 'matplotlib2.backends.backend_pgf',
    'png': 'matplotlib2.backends.backend_agg',
    'raw': 'matplotlib2.backends.backend_agg',
    'rgba': 'matplotlib2.backends.backend_agg',
    'svg': 'matplotlib2.backends.backend_svg',
    'svgz': 'matplotlib2.backends.backend_svg',
}
...
class _Backend(object):
    ...
    def new_figure_manager(cls, num, *args, **kwargs):
        ...
        from matplotlib2.figure import Figure
    ...
    def show(cls, block=None):
        ...
        if block is None:
            ...
            from matplotlib2 import pyplot
            try:
    ...
    * :meth:`draw_quad_mesh`
    ...
    def open_group(self, s, gid=None):
        ...
        *gid* as the id of the group. Is only currently used by
        :mod:`~matplotlib2.backends.backend_svg`.
        """

    def close_group(self, s):
        """
        Close a grouping element with label *s*
        Is only currently used by :mod:`~matplotlib2.backends.backend_svg`
        """

    def draw_path(self, gc, path, transform, rgbFace=None):
        """
        Draws a :class:`~matplotlib2.path.Path` instance using the
    ...
    def draw_markers(self, gc, marker_path, marker_trans, path,
        ...
        marker_trans : `matplotlib2.transforms.Transform`
            An affine transform applied to the marker.

        trans : `matplotlib2.transforms.Transform`
    ...
    def draw_quad_mesh(self, gc, master_transform, meshWidth, meshHeight,
        ...
        from matplotlib2.collections import QuadMesh
    ...
    def draw_gouraud_triangle(self, gc, points, colors, transform):
        ...
        transform : `matplotlib2.transforms.Transform`
    ...
    def draw_image(self, gc, x, y, im, transform=None):
        ...
        transform : `matplotlib2.transforms.Affine2DBase`
            If and only if the concrete backend is written such that
            :meth:`option_scale_image` returns ``True``, an affine
            transformation *may* be passed to :meth:`draw_image`. It takes the
            form of a :class:`~matplotlib2.transforms.Affine2DBase` instance.
    ...
    def draw_text(self, gc, x, y, s, prop, angle, ismath=False, mtext=None):
        ...
        prop : `matplotlib2.font_manager.FontProperties`
            font properties

        angle : scalar
            the rotation angle in degrees

        mtext : `matplotlib2.text.Text`
    ...
    def _get_text_path_transform(self, x, y, s, prop, angle, ismath):
        ...
        usetex : bool
          If True, use matplotlib2 usetex mode.
    ...
    def get_text_width_height_descent(self, s, prop, ismath):
        ...
        :class:`~matplotlib2.font_manager.FontProperties` *prop*
        """
        if ismath == 'TeX':
            # todo: handle props
    ...
    def flipy(self):
        ...
        for drawing text (:mod:`matplotlib2.text`) and images
        (:mod:`matplotlib2.image`) only
        """
        return True
    ...
    def get_texmanager(self):
        """
        return the :class:`matplotlib2.texmanager.TexManager` instance
        """
        if self._texmanager is None:
            from matplotlib2.texmanager import TexManager
...
class GraphicsContextBase(object):
    ...
    def get_clip_rectangle(self):
        """
        Return the clip rectangle as a :class:`~matplotlib2.transforms.Bbox`
        instance
        """
    ...
    def get_clip_path(self):
        ...
        is a :class:`~matplotlib2.path.Path` instance, and transform is
    ...
    def set_clip_path(self, path):
        ...
        :class:`~matplotlib2.transforms.TransformedPath` instance.
        """
        if (path is not None
                and not isinstance(path, transforms.TransformedPath)):
            raise ValueError("Path should be a "
                             "matplotlib2.transforms.TransformedPath instance")
        self._clippath = path
...
class Event(object):
    """
    A matplotlib2 event.  Attach additional attributes as defined in
    ...
    guiEvent
        the GUI event that triggered the matplotlib2 event
...
class LocationEvent(Event):
    ...
    inaxes : bool
        the :class:`~matplotlib2.axes.Axes` instance if mouse is over axes
...
class PickEvent(Event):
    ...
    artist : `matplotlib2.artist.Artist`
        the picked artist

    other
        extra class dependent attrs -- e.g., a
        :class:`~matplotlib2.lines.Line2D` pick may define different
        extra attributes than a
        :class:`~matplotlib2.collections.PatchCollection` pick event
...
class FigureCanvasBase(object):
    ...
    Attributes
    ----------
    figure : `matplotlib2.figure.Figure`
    ...
    if _has_pil:
        # JPEG support
        register_backend('jpg', 'matplotlib2.backends.backend_agg',
                         'Joint Photographic Experts Group')
        register_backend('jpeg', 'matplotlib2.backends.backend_agg',
                         'Joint Photographic Experts Group')
        # TIFF support
        register_backend('tif', 'matplotlib2.backends.backend_agg',
                         'Tagged Image File Format')
        register_backend('tiff', 'matplotlib2.backends.backend_agg',
                         'Tagged Image File Format')
    ...
    def draw(self, *args, **kwargs):
        """Render the :class:`~matplotlib2.figure.Figure`."""
    ...
    def mpl_connect(self, s, func):
        ...
        where event is a :class:`matplotlib2.backend_bases.Event`.  The
        ...
        set to the :class:`~matplotlib2.axes.Axes` the event occurs is
        over, and additionally, the variables ``event.xdata`` and
        ``event.ydata`` will be defined.  This is the mouse location
        in data coords.  See
        :class:`~matplotlib2.backend_bases.KeyEvent` and
        :class:`~matplotlib2.backend_bases.MouseEvent` for more info.

        Return value is a connection id that can be used with
        :meth:`~matplotlib2.backend_bases.Event.mpl_disconnect`.
        ...
        if s == 'idle_event':
            cbook.warn_deprecated(1.5,
                ...
                "be removed in matplotlib2 2.1. Use the animations module "
                "instead.")
...
def key_press_handler(event, canvas, toolbar=None):
    ...
    # Load key-mappings from your matplotlib2rc file.
...
class FigureManagerBase(object):
    ...
    def show(self):
        ...
        by :meth:`~matplotlib2.figure.Figure.show`, for an
        optional warning.
        """
