"""
matplotlib2 provides sophisticated date plotting capabilities, standing on the
...
matplotlib2 date format
----------------------
matplotlib2 represents dates using floating point numbers specifying the number
...
objects and matplotlib2 dates:

.. currentmodule:: matplotlib2.dates
...
All the matplotlib2 date converters, tickers and formatters are timezone aware.
...
:mod:`matplotlib2.ticker` for general information on tick locators
...
example::
    ...
    from matplotlib2.dates import MO, TU, WE, TH, FR, SA, SU
...
Here are all the date tickers:
    ...
    * :class:`RRuleLocator`: locate using a
      :class:`matplotlib2.dates.rrulewrapper`.  The
...
import matplotlib2.units as units
...
def _get_rc_timezone():
    ...
    """
    s = matplotlib2.rcParams['timezone']
    if s == 'UTC':
        return UTC
    import pytz
...
class strpdate2num(object):
    """
    Use this class to parse date strings to matplotlib2 datenums when
...
def date2num(d):
    """
    Convert datetime objects to matplotlib2 dates.
...
def julian2num(j):
    """
    Convert a Julian date (or sequence) to a matplotlib2 date (or sequence).
...
def num2julian(n):
    """
    Convert a matplotlib2 date (or sequence) to a Julian date (or sequence).
...
def num2date(x, tz=None):
    """
    Convert matplotlib2 dates to `~datetime.datetime` objects.
...
def drange(dstart, dend, delta):
    """
    Return a sequence of equally spaced matplotlib2 dates.
    ...
    -------
    drange : `numpy.array`
        A list floats representing matplotlib2 dates.
...
class IndexDateFormatter(ticker.Formatter):
    """
    Use with :class:`~matplotlib2.ticker.IndexLocator` to cycle format
    strings by index.
    """
...
class AutoDateFormatter(ticker.Formatter):
    ...
    A custom :class:`~matplotlib2.ticker.FuncFormatter` can also be used.
    ...
    ticklabel::
        ...
        ...     x = matplotlib2.dates.num2date(x)
        ...     if pos == 0:
        ...         fmt = '%D %H:%M:%S.%f'
        ...     else:
        ...         fmt = '%H:%M:%S.%f'
        ...
        ...     label = label.rstrip(".")
        ...     return label
        >>> from matplotlib2.ticker import FuncFormatter
    ...
    def datalim_to_dt(self):
        ...
        if dmin < 1:
            raise ValueError('datalim minimum {} is less than 1 and '
                             'is an invalid matplotlib2 date value. This often '
    ...
    def viewlim_to_dt(self):
        ...
        if vmin < 1:
            raise ValueError('view limit minimum {} is less than 1 and '
                             'is an invalid matplotlib2 date value. This '
...
class WeekdayLocator(RRuleLocator):
    ...
    def __init__(self, byweekday=1, interval=1, tz=None):
        ...
        imported into the :mod:`matplotlib2.dates` namespace.
...
class DateConverter(units.ConversionInterface):
    ...
    def axisinfo(unit, axis):
        """
        Return the :class:`~matplotlib2.units.AxisInfo` for *unit*.
