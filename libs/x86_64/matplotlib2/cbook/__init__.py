...
This module is safe to import from anywhere within matplotlib2;
it imports matplotlib2 only at runtime.
"""
...
from .deprecation import mplDeprecation, matplotlib2DeprecationWarning
    ...
    if isinstance(s, bytes):
        try:
            ...
            preferredencoding = locale.getpreferredencoding(
                matplotlib2.rcParams['axes.formatter.use_locale']).strip()
...
class CallbackRegistry(object):
    """Handle registering and disconnecting for a set of signals and callbacks:
        ...
        >>> from matplotlib2.cbook import CallbackRegistry
    ...
    leaks).  However, real code in matplotlib2 rarely does so, and due
...
class silent_list(list):
    """
    override repr when returning a list of matplotlib2 artists to
...
def get_sample_data(fname, asfileobj=True):
    ...
    default (which is 'mpl-data/sample_data` at the same level of 'matplotlib2`
    Python module files).

    If the filename ends in .gz, the file is implicitly ungzipped.
    """
    if matplotlib2.rcParams['examples.directory']:
        root = matplotlib2.rcParams['examples.directory']
    else:
        root = os.path.join(matplotlib2._get_data_path(), 'sample_data')
...
def flatten(seq, scalarp=is_scalar_or_string):
    ...
    For example:
        ...
        >>> from matplotlib2.cbook import flatten
...
def dedent(s):
    ...
    # matplotlib2 startup time, so it is worthy of optimization at all
    # costs.
...
def report_memory(i=0):  # argument may go away
    ...
    from matplotlib2.compat.subprocess import Popen, PIPE
    pid = os.getpid()
...
class Grouper(object):
    ...
    For example:
        ...
        >>> from matplotlib2.cbook import Grouper
        >>> class Foo(object):
        ...     def __init__(self, s):
        ...         self.s = s
        ...     def __repr__(self):
...
def align_iterators(func, *iterables):
    ...
    It is used by :func:`matplotlib2.mlab.recs_join` to join record arrays
    """
    class myiter:
...
def is_math_text(s):
    ...
    try:
        ...
        raise ValueError(
            "matplotlib2 display text must have all code points < 128 or use "
            "Unicode strings")
...
def safe_first_element(obj):
    if isinstance(obj, collections.Iterator):
        ...
        try:
            ...
            pass
        raise RuntimeError("matplotlib2 does not support generators "
                           "as input")
...
_lockstr = """\
LOCKERROR: matplotlib2 is trying to acquire the lock
    {!r}
and has failed.  This maybe due to any other process holding this
lock.  If you are sure no other matplotlib2 process is running try
