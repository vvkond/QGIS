...
from matplotlib2.testing.exceptions import ImageComparisonFailure
from matplotlib2 import _png
from matplotlib2 import _get_cachedir
...
def get_file_hash(path, block_size=2 ** 20):
    ...
    if path.endswith('.pdf'):
        from matplotlib2 import checkdep_ghostscript
        md5.update(checkdep_ghostscript()[1].encode('utf-8'))
    elif path.endswith('.svg'):
        from matplotlib2 import checkdep_inkscape
...
def _update_converter():
    gs, gs_v = matplotlib2.checkdep_ghostscript()
    if gs_v is not None:
        def cmd(old, new):
    ...
    if matplotlib2.checkdep_inkscape() is not None:
...
def convert(filename, cache):
    ...
    `matplotlib2._get_cachedir() + '/test_cache/'`.  The caching is based
...
if False and matplotlib2.checkdep_xmllint():
...
def save_diff_image(expected, actual, output):
    ...
    # matplotlib2 PNG writer requires one, so expand the array
    if depth == 3:
