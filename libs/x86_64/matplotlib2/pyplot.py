...
"""
`matplotlib2.pyplot` is a state-based interface to matplotlib2. It provides
...
import matplotlib2
import matplotlib2.colorbar
from matplotlib2 import style
from matplotlib2 import _pylab_helpers, interactive
from matplotlib2.cbook import dedent, silent_list, is_numlike
from matplotlib2.cbook import _string_to_bool
from matplotlib2.cbook import deprecated, warn_deprecated
from matplotlib2 import docstring
from matplotlib2.backend_bases import FigureCanvasBase
from matplotlib2.figure import Figure, figaspect
from matplotlib2.gridspec import GridSpec
from matplotlib2.image import imread as _imread
from matplotlib2.image import imsave as _imsave
from matplotlib2 import rcParams, rcParamsDefault, get_backend
from matplotlib2 import rc_context
from matplotlib2.rcsetup import interactive_bk as _interactive_bk
from matplotlib2.artist import getp, get, Artist
from matplotlib2.artist import setp as _setp
from matplotlib2.axes import Axes, Subplot
from matplotlib2.projections import PolarAxes
from matplotlib2 import mlab  # for csv2rec, detrend_none, window_hanning
from matplotlib2.scale import get_scale_docs, get_scale_names

from matplotlib2 import cm
from matplotlib2.cm import get_cmap, register_cmap
...
from matplotlib2.text import Text, Annotation
from matplotlib2.patches import Polygon, Rectangle, Circle, Arrow
from matplotlib2.widgets import SubplotTool, Button, Slider, Widget
...
from .ticker import TickHelper, Formatter, FixedFormatter, NullFormatter,\
           ...
           MaxNLocator
from matplotlib2.backends import pylab_setup
...
def uninstall_repl_displayhook():
    """
    Uninstalls the matplotlib2 display hook.
    ...
    .. warning
       ...
       function was there when matplotlib2 installed it's displayhook,
...
def switch_backend(newbackend):
    ...
    matplotlib2.use(newbackend, warn=False, force=True)
    from matplotlib2.backends import pylab_setup
    _backend_mod, new_figure_manager, draw_if_interactive, _show = pylab_setup()
...
def isinteractive():
    ...
    """
    return matplotlib2.is_interactive()


def ioff():
    """Turn interactive mode off."""
    matplotlib2.interactive(False)
    uninstall_repl_displayhook()


def ion():
    """Turn interactive mode on."""
    matplotlib2.interactive(True)
...
@docstring.copy_dedent(matplotlib2.rc)
def rc(*args, **kwargs):
    matplotlib2.rc(*args, **kwargs)


@docstring.copy_dedent(matplotlib2.rc_context)
def rc_context(rc=None, fname=None):
    return matplotlib2.rc_context(rc, fname)


@docstring.copy_dedent(matplotlib2.rcdefaults)
def rcdefaults():
    matplotlib2.rcdefaults()
    if matplotlib2.is_interactive():
        draw_all()
...
def gci():
    ...
    current :class:`~matplotlib2.cm.ScalarMappable` instance (image or
    patch collection), or *None* if no images or patch collections
    have been defined.  The commands :func:`~matplotlib2.pyplot.imshow`
    and :func:`~matplotlib2.pyplot.figimage` create
    :class:`~matplotlib2.image.Image` instances, and the commands
    :func:`~matplotlib2.pyplot.pcolor` and
    :func:`~matplotlib2.pyplot.scatter` create
    :class:`~matplotlib2.collections.Collection` instances.  The
...
def sci(im):
    ...
    commands like :func:`~matplotlib2.pyplot.jet`,
    :func:`~matplotlib2.pyplot.hot` or
    :func:`~matplotlib2.pyplot.clim`).  The current image is an
...
def xkcd(scale=1, length=100, randomness=2):
    ...
    from matplotlib2 import patheffects
    return rc_context({
...
def figure(num=None,  # autoincrement if None, else integer from 1-N
    ...
    FigureClass : class derived from matplotlib2.figure.Figure
    ...
    if figManager is None:
        ...
        if (max_open_warning >= 1 and len(allnums) >= max_open_warning):
            warnings.warn(
                ...
                "(`matplotlib2.pyplot.figure`) are retained until "
...
def _auto_draw_if_interactive(fig, val):
    ...
    """
    if val and matplotlib2.is_interactive() and not fig.canvas.is_saving():
...
def draw():
    ...
    :class:`~matplotlib2.figure.Figure` instance, :attr:`fig`, that
    was created using a :mod:`~matplotlib2.pyplot` function, is::
...
def figlegend(*args, **kwargs):
    ...
    *handles*
      a sequence of :class:`~matplotlib2.lines.Line2D` or
      :class:`~matplotlib2.patches.Patch` instances

    *loc*
      can be a string or an integer specifying the legend
      location

    A :class:`matplotlib2.legend.Legend` instance is returned.
    ...
    .. seealso::
       ...
       :func:`~matplotlib2.pyplot.legend`
...
def axes(arg=None, **kwargs):
    ...
    arg : None or 4-tuple or Axes
        ...
        - `~matplotlib2.axes.Axes`: This is equivalent to `.pyplot.sca`.
...
def gca(**kwargs):
    """
    Get the current :class:`~matplotlib2.axes.Axes` instance on the
    ...
    See Also
    --------
    matplotlib2.figure.Figure.gca : The figure's gca method.
    """
...
def subplot(*args, **kwargs):
    ...
    In the current figure, create and return an `~matplotlib2.axes.Axes`,
    ...
    `matplotlib2.axes.Axes` at the top right corner of the current figure,
    ...
    .. note::
       ...
       :meth:`~matplotlib2.figure.Figure.add_subplot` method or the
       :func:`~matplotlib2.pyplot.axes` function instead.

    Keyword arguments:

      *facecolor*:
        The background color of the subplot, which can be any valid
        color specifier.  See :mod:`matplotlib2.colors` for more
        information.
      ...
      *projection*:
        ...
        registered. See :mod:`matplotlib2.projections`.

    .. seealso::

        :func:`~matplotlib2.pyplot.axes`
...
def subplots(nrows=1, ncols=1, sharex=False, sharey=False, squeeze=True,
    ...
    sharex, sharey : bool or {'none', 'all', 'row', 'col'}, default: False
        ...
        on, use :meth:`~matplotlib2.axes.Axes.tick_params`.
    ...
    subplot_kw : dict, optional
        ...
        :meth:`~matplotlib2.figure.Figure.add_subplot` call used to create each
        subplot.

    gridspec_kw : dict, optional
        Dict with keywords passed to the
        :class:`~matplotlib2.gridspec.GridSpec` constructor used to create the
    ...
    **fig_kw :
    ...
    Returns
    -------
    fig : :class:`matplotlib2.figure.Figure` object

    ax : Axes object or array of Axes objects.

        ax can be either a single :class:`matplotlib2.axes.Axes` object or an
...
def subplot_tool(targetfig=None):
    ...
    A :class:`matplotlib2.widgets.SubplotTool` instance is returned.
    """
...
def box(on=None):
    ...
    ----------
    on : bool or None
        The new `~matplotlib2.axes.Axes` box state. If ``None``, toggle
        the state.

    See Also
    --------
    :meth:`matplotlib2.axes.Axes.set_frame_on`
    :meth:`matplotlib2.axes.Axes.get_frame_on`
    """
    ax = gca()
    if on is None:
...
def title(s, *args, **kwargs):
    ...
    .. seealso::
        See :func:`~matplotlib2.pyplot.text` for adding text
        to the current axes
...
def rgrids(*args, **kwargs):
    ...
    gridlines (:class:`~matplotlib2.lines.Line2D` instances) and
    *labels* is an array of tick labels
    (:class:`~matplotlib2.text.Text` instances). When called with
...
def thetagrids(*args, **kwargs):
    ...
    (:class:`~matplotlib2.lines.Line2D` instances) and *labels* is an
    array of tick labels (:class:`~matplotlib2.text.Text` instances)::
    ...
    *frac* is the fraction of the polar axes radius at which to place
    ...
    Return value is a list of tuples (*lines*, *labels*):
      ...
      - *lines* are :class:`~matplotlib2.lines.Line2D` instances

      - *labels* are :class:`~matplotlib2.text.Text` instances.

    Note that on input, the *labels* argument is a list of strings,
    and on output it is a list of :class:`~matplotlib2.text.Text`
    instances.
...
def colormaps():
    """
    matplotlib2 provides a number of colormaps, and others can be added using
    :func:`~matplotlib2.cm.register_cmap`.  This function documents the built-in
    ...
    matplotlib2 ships with 4 perceptually uniform color maps which are
...
def colorbar(mappable=None, cax=None, ax=None, **kw):
    ...
    return ret
colorbar.__doc__ = matplotlib2.colorbar.colorbar_doc
...
def set_cmap(cmap):
    ...
    *cmap* must be a :class:`~matplotlib2.colors.Colormap` instance, or
    the name of a registered colormap.

    See :func:`matplotlib2.cm.register_cmap` and
    :func:`matplotlib2.cm.get_cmap`.
    """
...
def polar(*args, **kwargs):
    ...
    strings, as in :func:`~matplotlib2.pyplot.plot`.
...
def plotfile(fname, cols=(0,), plotfuncs=None,
    ...
    *cols* is a sequence of column identifiers to plot.  An identifier
    ...
    matplotlib2 will make column headers lower case, replace spaces with
    ...
    *plotfuncs*, if not *None*, is a dictionary mapping identifier to
    an :class:`~matplotlib2.axes.Axes` plotting function as a string.
    ...
    *comments*, *skiprows*, *checkrows*, *delimiter*, and *names*
    are all passed on to :func:`matplotlib2.pylab.csv2rec` to
    ...
    interface to general plotting with pyplot or matplotlib2.
    """
