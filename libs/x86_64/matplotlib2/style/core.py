...
"""
Core functions and attributes for the matplotlib2 style library:

``use``
    Select style sheet to override the current matplotlib2 settings.
``context``
    Context manager to use a style sheet temporarily.
``available``
    List available style sheets.
``library``
    A dictionary of style names and matplotlib2 settings.
"""
import os
import re
import contextlib
...
from matplotlib2 import rc_params_from_file, rcParamsDefault
...
def use(style):
    """Use matplotlib2 style settings from a style specification.
