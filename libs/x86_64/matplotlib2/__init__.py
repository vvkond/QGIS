...
at your terminal, followed by::
    ...
    In [1]: %matplotlib2
    In [2]: import matplotlib2.pyplot as plt
...
interactively.  The
exceptions are the pyplot commands :func:`~matplotlib2.pyplot.figure`,
:func:`~matplotlib2.pyplot.subplot`,
:func:`~matplotlib2.pyplot.subplots`, and
:func:`~pyplot.savefig`, which can greatly simplify scripting.

Modules include:

    :mod:`matplotlib2.axes`
        defines the :class:`~matplotlib2.axes.Axes` class.  Most pylab
        commands are wrappers for :class:`~matplotlib2.axes.Axes`
        methods.  The axes module is the highest level of OO access to
        the library.

    :mod:`matplotlib2.figure`
        defines the :class:`~matplotlib2.figure.Figure` class.

    :mod:`matplotlib2.artist`
        defines the :class:`~matplotlib2.artist.Artist` base class for
        all classes that draw things.

    :mod:`matplotlib2.lines`
        defines the :class:`~matplotlib2.lines.Line2D` class for
        drawing lines and markers

    :mod:`matplotlib2.patches`
        defines classes for drawing polygons

    :mod:`matplotlib2.text`
        defines the :class:`~matplotlib2.text.Text`,
        :class:`~matplotlib2.text.TextWithDash`, and
        :class:`~matplotlib2.text.Annotate` classes

    :mod:`matplotlib2.image`
        defines the :class:`~matplotlib2.image.AxesImage` and
        :class:`~matplotlib2.image.FigureImage` classes

    :mod:`matplotlib2.collections`
        classes for efficient drawing of groups of lines or polygons

    :mod:`matplotlib2.colors`
        classes for interpreting color specifications and for making
        colormaps

    :mod:`matplotlib2.cm`
        colormaps and the :class:`~matplotlib2.image.ScalarMappable`
        mixin class for providing color mapping functionality to other
        classes

    :mod:`matplotlib2.ticker`
        classes for calculating tick mark locations and for formatting
        tick labels

    :mod:`matplotlib2.backends`
        a subpackage with modules for various gui libraries and output
        formats

The base matplotlib2 namespace includes:

    :data:`~matplotlib2.rcParams`
        a global dictionary of default configuration settings.  It is
        initialized by code which may be overridden by a matplotlib2rc
        file.

    :func:`~matplotlib2.rc`
        a function for setting groups of rcParams values

    :func:`~matplotlib2.use`
        a function for setting the matplotlib2 backend.  If used, this
        function must be called immediately after importing matplotlib2
        for the first time.  In particular, it must be called
        **before** importing pylab (if pylab is imported).

matplotlib2 was initially written by John D. Hunter (1968-2012) and is now
...
# cbook must import matplotlib2 only within function
...
from matplotlib2.rcsetup import defaultParams, validate_backend, cycler
...
__bibtex__ = r"""@Article{Hunter:2007,
  Author    = {Hunter, J. D.},
  Title     = {matplotlib2: A 2D graphics environment},
  Journal   = {Computing In Science \& Engineering},
  Volume    = {9},
  Number    = {3},
  Pages     = {90--95},
  abstract  = {matplotlib2 is a 2D graphics package used for Python
...
if not (_python27 or _python34):
    raise ImportError("matplotlib2 requires Python 2.7 or 3.4 or later")
...
try:
    ...
    raise ImportError("matplotlib2 requires dateutil")


if not compare_versions(six.__version__, '1.10'):
    raise ImportError(
        "matplotlib2 requires six>=1.10; you have %s" % six.__version__)


try:
    import pyparsing
except ImportError:
    raise ImportError("matplotlib2 requires pyparsing")
else:
    if not compare_versions(pyparsing.__version__, '2.0.1'):
        raise ImportError(
            "matplotlib2 requires pyparsing>=2.0.1; you have %s"
            % pyparsing.__version__)


if not compare_versions(numpy.__version__, __version__numpy__):
    raise ImportError(
        "matplotlib2 requires numpy>=%s; you have %s" % (
...
_verbose_msg = """\
matplotlib2.verbose is deprecated;
Command line argument --verbose-LEVEL is deprecated.
This functionality is now provided by the standard
python logging library.  To get more (or less) logging output:
    import logging
    logger = logging.getLogger('matplotlib2')
...
class Verbose(object):
    ...
    def set_level(self, level):
        ...
        if level not in self.levels:
            warnings.warn('matplotlib2: unrecognized --verbose-* string "%s".'
...
def checkdep_ps_distiller(s):
    ...
    if not compare_versions(gs_v, gs_req):
        flag = False
        warnings.warn(('matplotlib2rc ps.usedistiller option can not be used '
    ...
    if s == 'xpdf':
        ...
        if compare_versions(pdftops_v, pdftops_req):
            ...
            flag = False
            warnings.warn(('matplotlib2rc ps.usedistiller can not be set to '
...
def checkdep_usetex(s):
    ...
    if _backports.which("tex") is None:
        flag = False
        warnings.warn('matplotlib2rc text.usetex option can not be used unless '
                      'TeX is installed on your system')

    dvipng_v = checkdep_dvipng()
    if not compare_versions(dvipng_v, dvipng_req):
        flag = False
        warnings.warn('matplotlib2rc text.usetex can not be used with *Agg '
    ...
    if not compare_versions(gs_v, gs_req):
        flag = False
        warnings.warn('matplotlib2rc text.usetex can not be used unless '
...
def _create_tmp_config_dir():
    ...
    configdir = os.environ['MPLCONFIGDIR'] = (
        tempfile.mkdtemp(prefix='matplotlib2-'))
...
def _get_config_or_cache_dir(xdg_base):
    from matplotlib2.cbook import mkdirs
    ...
    h = get_home()
    if h is not None:
        p = os.path.join(h, '.matplotlib2')
    if sys.platform.startswith(('linux', 'freebsd')):
        p = None
        if xdg_base is not None:
            p = os.path.join(xdg_base, 'matplotlib2')
...
def _get_configdir():
    ...
    2b. On other platforms, choose `$HOME/.matplotlib2`.
...
def _get_data_path():
    'get the path to matplotlib2 data'

    if 'matplotlib2DATA' in os.environ:
        path = os.environ['matplotlib2DATA']
        if not os.path.isdir(path):
            raise RuntimeError('Path in environment matplotlib2DATA not a '
                               'directory')
        return path
    ...
    # so need to try something known to be in matplotlib2, not basemap
    import matplotlib2.afm
    _file = _decode_filesystem_path(matplotlib2.afm.__file__)
    path = os.sep.join([os.path.dirname(_file), 'mpl-data'])
    if os.path.isdir(path):
        return path
    ...
    raise RuntimeError('Could not find the matplotlib2 data files')
...
get_data_path = _wrap('matplotlib2 data path %s', _get_data_path_cached,
                      always=False)
...
def get_py2exe_datafiles():
    ...
    for root, _, files in os.walk(datapath):
        ...
        if 'matplotlib2.nib' in files:
            files.remove('matplotlib2.nib')
...
def matplotlib2_fname():
    ...
    - `$matplotlib2RC` if it is a file (or a named pipe, which can be created
      e.g. by process substitution)

    - `$matplotlib2RC/matplotlib2rc`

    - `$MPLCONFIGDIR/matplotlib2rc`

    - On Linux,

          - `$XDG_CONFIG_HOME/matplotlib2/matplotlib2rc` (if
            $XDG_CONFIG_HOME is defined)

          - or `$HOME/.config/matplotlib2/matplotlib2rc` (if
            $XDG_CONFIG_HOME is not defined)

    - On other platforms,

         - `$HOME/.matplotlib2/matplotlib2rc` if `$HOME` is defined.

    - Lastly, it looks in `$matplotlib2DATA/matplotlib2rc` for a
      system-defined copy.
    """
...
class RcParams(MutableMapping, dict):
    ...
    msg_depr_set = ("%s is deprecated. Please remove it from your "
                    "matplotlib2rc and/or style files.")
    msg_depr_ignore = "%s is deprecated and ignored. Use %s instead."
    msg_obsolete = ("%s is obsolete. Please remove it from your matplotlib2rc "
                    "and/or style files.")
...
def rc_params(fail_on_error=False):
    """Return a :class:`matplotlib2.RcParams` instance from the
    default matplotlib2 rc file.
    """
    fname = matplotlib2_fname()
...
def _rc_params_in_file(fname, fail_on_error=False):
    """Return :class:`matplotlib2.RcParams` from the contents of the given file.
    ...
    for key, (val, line, cnt) in six.iteritems(rc_temp):
        if key in defaultParams:
            ...
            warnings.warn('%s is deprecated. Update your matplotlib2rc to use '
...
%s.
You probably need to get an updated matplotlib2rc file from
http://github.com/matplotlib2/matplotlib2/blob/master/matplotlib2rc.template
or from the matplotlib2 source distribution""" % (key, cnt, fname),
                  file=sys.stderr)
...
def rc_params_from_file(fname, fail_on_error=False, use_default_template=True):
    ...
    ----------
    fname : str
        Name of file parsed for matplotlib2 settings.
    fail_on_error : bool
...
# this is the instance used by the matplotlib2 classes
rcParams = rc_params()

if rcParams['examples.directory']:
    # paths that are intended to be relative to matplotlib2_fname()
    # are allowed for the examples.directory parameter.
    # However, we will need to fully qualify the path because
    # Sphinx requires absolute paths.
    if not os.path.isabs(rcParams['examples.directory']):
        _basedir, _fname = os.path.split(matplotlib2_fname())
        # Sometimes matplotlib2_fname() can return relative paths,
        # Also, using realpath() guarantees that Sphinx will use
        # the same path that matplotlib2 sees (in case of weird symlinks).
...
def rc(group, **kwargs):
    ...
    *kwargs* is a dictionary attribute name/value pairs, e.g.,::
    ...
    ``matplotlib2.style.use('default')`` or :func:`~matplotlib2.rcdefaults` to
...
def rcdefaults():
    """Restore the rc params from matplotlib2's internal defaults.

    See Also
    --------
    rc_file_defaults :
        Restore the rc params from the rc file originally loaded by matplotlib2.
    matplotlib2.style.use :
...
def rc_file_defaults():
    """Restore the rc params from the original rc file loaded by matplotlib2.
    """
...
_use_error_msg = """
This call to matplotlib2.use() has no effect because the backend has already
been chosen; matplotlib2.use() must be called *before* pylab, matplotlib2.pyplot,
or matplotlib2.backends is imported for the first time.
...
def use(arg, warn=True, force=False):
    """
    Set the matplotlib2 backend to one of the known backends.

    The argument is case-insensitive. *warn* specifies whether a
    warning should be issued if a backend has already been set up.
    *force* is an **experimental** flag that tells matplotlib2 to
    ...
    .. note::
        ...
        before importing matplotlib2.backends.  If warn is True, a warning
    ...
    if 'matplotlib2.backends' in sys.modules:
        # Warn only if called with a different name
        if (rcParams['backend'] != name) and warn:
            import matplotlib2.backends
            warnings.warn(
                _use_error_msg.format(
                    backend=rcParams['backend'],
                    tb=matplotlib2.backends._backend_loading_tb),
                stacklevel=2)
    ...
    if need_reload:
        reload(sys.modules['matplotlib2.backends'])
...
def _init_tests():
    ...
    from matplotlib2 import ft2font
    if (ft2font.__freetype_version__ != LOCAL_FREETYPE_VERSION or
        ft2font.__freetype_build_type__ != 'local'):
        warnings.warn(
            "matplotlib2 is not built with the correct FreeType version to run "
    ...
    try:
        ...
        print("matplotlib2.test requires pytest and mock to run.")
        raise


def test(verbosity=None, coverage=False, switch_backend_warn=True,
         recursionlimit=0, **kwargs):
    """run the matplotlib2 test suite"""
    _init_tests()
    if not os.path.isdir(os.path.join(os.path.dirname(__file__), 'tests')):
        raise ImportError("matplotlib2 test data is not installed")
            ...
            if (label_namer and not user_supplied_label):
                if _label_namer_pos < len(args):
                    ...
                    warnings.warn(
                        ...
                        "the matplotlib2 list!)" % (label_namer, func.__name__),
...
_log.debug('matplotlib2 version %s', __version__)
