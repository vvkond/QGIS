...
customization using matplotlib2's rc settings.
...
These default values should be consistent with the default matplotlib2rc file
that actually reflects the values given here. Any additions or deletions to the
parameter set listed here should also be visited to the
:file:`matplotlib2rc.template` in matplotlib2's root source directory.
"""
...
from matplotlib2 import cbook, testing
from matplotlib2.cbook import mplDeprecation, deprecated, ls_mapper
from matplotlib2.fontconfig_pattern import parse_fontconfig_pattern
from matplotlib2.colors import is_color_like
...
def validate_ps_distiller(s):
    ...
    if s in ('none', None):
        ...
        raise ValueError('matplotlib2rc ps.usedistiller must either be none, '
                         'ghostscript or xpdf')
...
def validate_animation_writer_path(p):
    ...
    # trigger the checks.
    if "matplotlib2.animation" in modules:
        modules["matplotlib2.animation"].writers.set_dirty()
    return p
...
defaultParams = {
    ...
    # use matplotlib2 rc settings for font configuration
    ...
    'plugins.directory':  ['.matplotlib2_plugins', validate_string],
    ...
    # altogether.  For that use `matplotlib2.style.use('classic')`.
